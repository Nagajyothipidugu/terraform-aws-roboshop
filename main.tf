resource "aws_lb_target_group" "main" {
  name     = "${var.project}-${var.environment}-${var.component}" #roboshop-dev-component
  port     = local.tg_port
  protocol = "HTTP"
  vpc_id   = local.vpc_id
...
# configuration of instance by connecting and integrating ansible here
...
resource "aws_launch_template" "main" {
  ...
  instance_type = "t3.micro"
  vpc_security_group_ids = [local.sg_id]
  update_default_version = true  # Each time you update or run terraform apply,new version becomes default
  tag_specifications {
